...
// .wrangler/tmp/bundle-nr7rLH/strip-cf-connecting-ip-header.js
...
var sign3 = /* @__PURE__ */ notImplemented("crypto.sign");
var verify3 = /* @__PURE__ */ notImplemented("crypto.verify");
...
function hashPassword(password) {
  if (!password)
    return "";
  const salt = crypto_default.randomBytes(16).toString("hex");
  const hash2 = crypto_default.pbkdf2Sync(password, salt, 1e3, 64, "sha512").toString("hex");
  return `${salt}:${hash2}`;
}
...
function verifyPassword(password, storedHash) {
  ...
  try {
    ...
    if (!salt || !originalHash)
      return false;
    const hashToTest = crypto_default.pbkdf2Sync(password, salt, 1e3, 64, "sha512").toString("hex");
    return crypto_default.timingSafeEqual(Buffer.from(hashToTest), Buffer.from(originalHash));
  } catch (e) {
    return false;
  }
}
...
__name(objectToFirestoreFields, "objectToFirestoreFields");
function firestoreFieldsToObject(fields = {}) {
  ...
  for (const [key, valObj] of Object.entries(fields)) {
    if ("stringValue" in valObj) {
      ...
      res[key] = Number(valObj.integerValue);
    else if ("doubleValue" in valObj)
      res[key] = Number(valObj.doubleValue);
    else if ("booleanValue" in valObj)
      res[key] = Boolean(valObj.booleanValue);
    else
      res[key] = valObj;
  }
  return res;
}
__name(firestoreFieldsToObject, "firestoreFieldsToObject");
async function signJwtRs256(clientEmail, privateKeyPem) {
  const now = Math.floor(Date.now() / 1e3);
  ...
  const cryptoKey = await crypto_default.subtle.importKey(
    "pkcs8",
    binaryDer,
  ...
  const signatureBuffer = await crypto_default.subtle.sign(
    "RSASSA-PKCS1-v1_5",
    cryptoKey,
...
var cachedFirestoreToken = null;
async function getFirestoreAccessToken(env2) {
  const now = Math.floor(Date.now() / 1e3);
  if (cachedFirestoreToken && cachedFirestoreToken.expiresAt > now + 60) {
    return cachedFirestoreToken.token;
  }
  const clientEmail = env2.FIREBASE_CLIENT_EMAIL || process.env.FIREBASE_CLIENT_EMAIL || "";
  const privateKey = env2.FIREBASE_PRIVATE_KEY || process.env.FIREBASE_PRIVATE_KEY || "";
  if (!clientEmail || !privateKey) {
    throw new Error("Firebase service account credentials missing in environment");
  }
  const jwt2 = await signJwtRs256(clientEmail, privateKey);
...
async function queryFirestore(collection, field, value, env2) {
  const projectId = env2.FIREBASE_PROJECT_ID || process.env.FIREBASE_PROJECT_ID;
  if (!projectId)
    throw new Error("FIREBASE_PROJECT_ID missing");
  const token = await getFirestoreAccessToken(env2);
...
async function addFirestoreDoc(collection, data, env2) {
  const projectId = env2.FIREBASE_PROJECT_ID || process.env.FIREBASE_PROJECT_ID;
  if (!projectId)
    throw new Error("FIREBASE_PROJECT_ID missing");
  const token = await getFirestoreAccessToken(env2);
...
__name(addFirestoreDoc, "addFirestoreDoc");
async function getFirestoreDoc(collection, docId, env2) {
  const projectId = env2.FIREBASE_PROJECT_ID || process.env.FIREBASE_PROJECT_ID;
  if (!projectId)
    throw new Error("FIREBASE_PROJECT_ID missing");
  const token = await getFirestoreAccessToken(env2);
...
async function updateFirestoreDoc(collection, docId, data, env2) {
  const projectId = env2.FIREBASE_PROJECT_ID || process.env.FIREBASE_PROJECT_ID;
  if (!projectId)
    throw new Error("FIREBASE_PROJECT_ID missing");
  const token = await getFirestoreAccessToken(env2);
...
async function deleteFirestoreDoc(collection, docId, env2) {
  const projectId = env2.FIREBASE_PROJECT_ID || process.env.FIREBASE_PROJECT_ID;
  if (!projectId)
    throw new Error("FIREBASE_PROJECT_ID missing");
  const token = await getFirestoreAccessToken(env2);
...
async function listFirestoreDocs(collection, env2) {
  const projectId = env2.FIREBASE_PROJECT_ID || process.env.FIREBASE_PROJECT_ID;
  if (!projectId)
    throw new Error("FIREBASE_PROJECT_ID missing");
  const token = await getFirestoreAccessToken(env2);
...
__name(listFirestoreDocs, "listFirestoreDocs");
async function sendEmailViaGmailRest(to, subject, htmlContent, env2) {
  const clientId = env2.GOOGLE_CLIENT_ID || process.env.GOOGLE_CLIENT_ID;
  const clientSecret = env2.GOOGLE_CLIENT_SECRET || process.env.GOOGLE_CLIENT_SECRET;
  const refreshToken = env2.GOOGLE_REFRESH_TOKEN || process.env.GOOGLE_REFRESH_TOKEN;
  ...
  const fromEmail = env2.EMAIL_FROM || process.env.EMAIL_FROM || "Endeavour SLIET <ubuntu.surya@gmail.com>";
  const rawMessage = [
    `From: ${fromEmail}`,
    `To: ${to}`,
    `Subject: ${subject}`,
...
__name(sendEmailViaGmailRest, "sendEmailViaGmailRest");
async function candidateAuthMiddleware(c, next) {
...
__name(candidateAuthMiddleware, "candidateAuthMiddleware");
...
app.post("/api/upload", async (c) => {
  try {
    ...
    const signature = crypto_default.createHash("sha1").update(strToSign).digest("hex");
...
app.post("/api/register", async (c) => {
  try {
    ...
    const rawPassword = body.Password || crypto_default.randomBytes(4).toString("hex").toLowerCase();
    const record = {
      ...body,
      Email: cleanEmail,
      RegNo: cleanRegNo,
...
var handleCandidateLogin = /* @__PURE__ */ __name(async (c) => {
  try {
    ...
    const token = await sign2(tokenPayload, jwtSecret, "HS256");
...
var handleCandidateProfileGet = /* @__PURE__ */ __name(async (c) => {
  try {
...
var handleCandidateProfilePut = /* @__PURE__ */ __name(async (c) => {
  try {
    const jwtPayload = c.get("jwtPayload");
    ...
    updateFields.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
...
app.put("/api/candidate/change-password", candidateAuthMiddleware, async (c) => {
  try {
    ...
    await updateFirestoreDoc("registrations", jwtPayload.id, { Password: newHashed, updatedAt: (/* @__PURE__ */ new Date()).toISOString() }, c.env);
...
app.post("/api/auth/forgot-password", async (c) => {
  try {
    ...
    const token = crypto_default.randomBytes(32).toString("hex");
...
app.post("/api/auth/reset-password", async (c) => {
  try {
    ...
    const newHashed = hashPassword(String(newPassword).trim());
    await updateFirestoreDoc("registrations", resetDoc.targetDocId, { Password: newHashed, updatedAt: (/* @__PURE__ */ new Date()).toISOString() }, c.env);
    await updateFirestoreDoc("password_resets", resetDoc.id, { used: true, usedAt: (/* @__PURE__ */ new Date()).toISOString() }, c.env);
...
app.post("/api/admin/login", async (c) => {
  try {
    ...
    const jwtSecret = c.env.JWT_SECRET || process.env.JWT_SECRET || "endeavour_secret_key_2025";
    const token = await sign2({ email: cleanInputEmail, role: "admin", exp: Math.floor(Date.now() / 1e3) + 86400 }, jwtSecret, "HS256");
...
app.post("/api/admin/registrations/batch-resend-email", adminAuthMiddleware, async (c) => {
  try {
    ...
    for (const id of ids) {
      ...
      if (doc && doc.Email) {
        const rawPassword = crypto_default.randomBytes(4).toString("hex").toLowerCase();
...
app.put("/api/admin/registrations/:id/marks", adminAuthMiddleware, async (c) => {
  try {
    ...
    if (!ok)
      return c.json({ error: "Failed to update marks" }, 500);
...
app.put("/api/admin/registrations/:id/status", adminAuthMiddleware, async (c) => {
  try {
    ...
    const ok = await updateFirestoreDoc("registrations", id, { status, updatedAt: (/* @__PURE__ */ new Date()).toISOString() }, c.env);
    if (!ok)
...
// .wrangler/tmp/bundle-nr7rLH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-nr7rLH/middleware-loader.entry.ts
